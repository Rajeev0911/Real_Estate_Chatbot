...
class RealEstateAgent:
    ...
    def _preprocess_knowledge_base(self):
        ...
        if 'Area' in self.knowledge_base.columns:
            self.knowledge_base['Area'] = self.knowledge_base['Area'].astype(str)
            self.knowledge_base['Area'] = self.knowledge_base['Area'].str.extract(r'(\d+(?:\.\d+)?)')  # Extract numeric values
            self.knowledge_base['Area'] = pd.to_numeric(self.knowledge_base['Area'], errors='coerce')  # Convert to numeric
...
"""
    ...
    def get_property_insights(self, properties):
        ...
        for _, prop in properties.iterrows():
            try:
                price = float(prop['Price']) if pd.notna(prop.get('Price')) else np.nan
                area = prop['Area'] if pd.notna(prop.get('Area')) else "N/A"
            ...
            insight = {
                ...
                # 'Furnishing': str(prop.get('Furnishing', 'N/A')),
                'Bedrooms': bedrooms,
                'Type': str(prop.get('Type', 'N/A')),
                'Area(sqft)': f"{area} sqft" if area != "N/A" else "N/A",
