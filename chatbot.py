...
#         # Accept user input when run directly
#         print("Welcome to the Real Estate Agent. Type 'exit' to quit.")
#         while True:
#             query = input("\nEnter your real estate query: ")
#             if query.lower() == 'exit':
#                 break
...
class RealEstateAgent:
    def __init__(self, gemini_api_key, csv_path):
        ...
        self.follow_ups = [
            ...
            "Are you interested in any specific amenities?",
...
"""
    ...
    def format_results_as_conversational(self, user_query, matching_properties):
        """Format results in a conversational, human-like manner."""
        if not matching_properties:
            return "I've searched for properties matching your criteria, but unfortunately, I couldn't find any that match exactly what you're looking for. Would you like to broaden your search? Perhaps we could adjust the budget, location, or number of bedrooms?"
        
        result = f"Great news! I found {len(matching_properties)} properties that match what you're looking for. Here are the highlights:\n\n"
        
        for i, prop in enumerate(matching_properties, 1):
            result += f"✨ Property {i}: {prop['Title']}\n"
            result += f"📍 Located in {prop['Location']}\n"
            result += f"💰 Priced at {prop['Price']} for {prop['Purpose']}\n"
            result += f"🏡 {prop['Type']} with {prop['Bedrooms']} bedrooms and {prop['Bathrooms']} bathrooms\n"
            result += f"🛋️ {prop['Furnishing']}\n"
            
            # Add a short excerpt from the description (first 100 characters)
            desc = prop['Description']
            if len(desc) > 100:
                desc = desc[:100] + "..."
            result += f"📝 {desc}\n\n"
        
        result += "Would you like more details about any of these properties? Or would you like to refine your search further?"
        
        return result
    
    def process_real_estate_query(self, user_query):
        """Process user query with conversational awareness."""
        # Check if this is a greeting or basic question
        if self._is_greeting(user_query):
            return random.choice(self.greetings)
        
        if self._is_basic_question(user_query):
            return "I'm your personal real estate assistant specialized in helping you find properties in Dubai. I can search for apartments, villas, houses based on your preferences like location, price, number of bedrooms, and amenities. Just tell me what you're looking for and I'll find the best matches for you!"
        
        # If query is too short or vague, ask for more details
        if len(user_query.split()) < 3:
            return f"I'd be happy to help you find a property. Could you please provide more details about what you're looking for? For example, location preferences, budget, number of bedrooms, or property type."
        ...
        if not query_details:
            return "I'm sorry, I'm having trouble understanding your request. Could you please rephrase it? For example, 'I'm looking for a 2-bedroom apartment in Downtown Dubai with a budget of 1.5 million AED.'"
        ...
        if filtered.empty:
            return "I've searched our database, but I couldn't find properties matching those exact criteria. Would you like to broaden your search? Perhaps consider a different location or adjusting your budget?"
    
        matching = self._semantic_search(filtered, user_query)
    
        if matching.empty:
            return "I found some properties that partially match your criteria, but none that are a great fit. Could you tell me which aspects are most important to you so I can focus on those?"
    
        insights = self.get_property_insights(matching)
    
        # Format results conversationally
        return self.format_results_as_conversational(user_query, insights)
...
#         # Accept user input when run directly
#         print("Welcome to your personal real estate assistant. I can
def main():
    # Replace with your actual Gemini API key and CSV file path
    GEMINI_API_KEY = "AIzaSyCHTlehGQFmbl9Cq9HAPKEGiON7CC8HILY"
    CSV_PATH = "realestatedata.csv"
